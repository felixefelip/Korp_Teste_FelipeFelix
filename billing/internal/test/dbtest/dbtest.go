...
func Setup() (*gorm.DB, error) {
	...
	if err := connection.AutoMigrate(&model.Invoice{}); err != nil {
...
func Reset(t testing.TB, connection *gorm.DB) {
	...
	err := connection.Exec("TRUNCATE TABLE invoice RESTART IDENTITY CASCADE").Error
	require.NoError(t, err, "cleaning the invoice table")
}
